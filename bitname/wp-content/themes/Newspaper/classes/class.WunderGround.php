    ...
    public function getAlertsFromDBByZip($zipcode, $radius, $date_start='', $date_end='') {
        ...
        if($wpdb) {
            ...
            if(empty($date_start) || empty($date_end)) {
                $sqlCheckAlert = "SELECT *,FROM_UNIXTIME(date_epoch) FROM alerts WHERE {$sqlWHERE} ORDER BY idalerts DESC";
            }
            else {
                $date_start = strtotime($date_start);
                $date_end = strtotime($date_end);
                $sqlCheckAlert = "SELECT *,FROM_UNIXTIME(date_epoch) FROM alerts WHERE {$sqlWHERE} AND (date_epoch BETWEEN {$date_start} AND {$date_end}) ORDER BY idalerts DESC";
            }
